...
# ✅ Python 3.14 fix: forceer event loop in MainThread (voorkomt "no current event loop")
try:
...
CHAT_ID = None  # straks: -100xxxxxxxxxx
...
async def log_updates(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Logt raw update JSON zodat je de chat.id van de groep ziet.
...
async def pinned_post_loop(app: Application):
    while True:
        ...
        if msg:
            ...
            safe_create_task(delete_later(app.bot, CHAT_ID, msg.message_id, DELETE_PIN_SECONDS), "delete_pinned_after_15s")
...
async def on_new_members(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Als CHAT_ID bekend is, alleen die groep
